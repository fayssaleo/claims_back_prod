        ...
        if ($request->id == 0) {
            ...
            if ($request->major) {
                ...
                DB::table('vessels')
                    ->where('claim_id', $request->claim_id)
                    ->update(['major' => 0]);
                /*$Tautomobiles = Automobile::select()
                    ->where('claim_id', $request->claim_id)
                    ->get();

                for ($i = 0; $i < count($Tautomobiles); $i++) {
                    $TthisEquipment = Automobile::find($Tautomobiles[$i])->first();
                    $TthisEquipment->major = false;
                    $TthisEquipment->save();
                }

                $Tequipments = Equipment::select()
                    ->where('claim_id', $request->claim_id)
                    ->get();
                for ($i = 0; $i < count($Tequipments); $i++) {
                    $TthisEquipment = Equipment::find($Tequipments[$i])->first();
                    $TthisEquipment->major = false;
                    $TthisEquipment->save();
                }

                $Tcontainers = Container::select()
                    ->where('claim_id', $request->claim_id)
                    ->get();
                for ($i = 0; $i < count($Tcontainers); $i++) {
                    $TthisContainer = Container::find($Tcontainers[$i])->first();
                    $TthisContainer->major = false;
                    $TthisContainer->save();
                }

                $Tvessels = Vessel::select()
                    ->where('claim_id', $request->claim_id)
                    ->get();
                for ($i = 0; $i < count($Tvessels); $i++) {
                    $TthisVessel = Vessel::find($Tvessels[$i])->first();
                    $TthisVessel->major = false;
                    $TthisVessel->save();
                }*/
            }
            if ($request->nature_of_damage["id"] == 0) {
